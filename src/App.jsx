...
function Overlay() {
  ...
  const [phase, setPhase] = useState('countdown'); // 'countdown' | 'roulette'

  // Handle countdown complete - user clicked in time
  const handleCountdownComplete = () => {
    console.log('[APP] Countdown complete - advancing to roulette');
    setPhase('roulette');
  };

  // Handle reset after timeout
  const handleReset = () => {
    console.log('[APP] Resetting to countdown');
    setPhase('countdown');
  };
  ...
  if (phase === 'countdown') {
    return (
      <div className={`transition-all duration-700 ease-in-out ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'}`}>
        <CountdownScreen
